...
export const OutlineButton: React.FC<ButtonProps> = ({
    ...
    return (
        <button
            {...props}
            disabled={disabled}
            className={`inline-flex items-center px-4 py-2 border-2 border-primary rounded-md font-semibold text-xs text-secondary-dark hover:bg-primary focus:outline-none active:bg-primary focus:border-tertiary transition ease-in-out duration-150 ${
                disabled && "opacity-25"
            } ${className}`}
        >
            {children}
